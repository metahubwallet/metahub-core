<template>
  <div @click="handleBGClick" class="box-container">
    ...
    <el-button :loading="submitLoading" @click="submit()" class="submit-button">{{ $t('wallet.transfer') }}</el-button>
...
export default {
  ...
  computed: {
    receiver() {
      const rsl = this.transfer.receiver.length;
      let account = this.transfer.receiver;
      if (rsl == 42) {
        account = this.transfer.receiver.substring(0, 14) + '...' + this.transfer.receiver.substring(rsl - 8);
      }
      return account.toLowerCase();
    },
    showMemo() {
      return this.transfer.receiver.length != 42;
    },
  },
  methods: {
    handleBGClick(event) {
